...
import {
  LayoutDashboard, Store, Package, ClipboardList, BarChart2, User, LogOut, Waves, Menu, X,
} from "lucide-react";
            ...
            <Link key={item.href} href={item.href} onClick={onClose}>
              <motion.div
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition ${
                  active ? "bg-orange-50 text-orange-600 border border-orange-100" : "text-gray-600 hover:bg-gray-100"
                }`}
              >
...
export default function SellerLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!token || user?.activeRole !== "SELLER") router.replace("/login");
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between px-4 h-14 border-b border-gray-100">
                <Link href="/" className="flex items-center gap-1.5 text-orange-500 font-bold">
                  <Waves className="w-4 h-4" /> Seapedia Seller
                </Link>
